...
die "The number of lines in one of the files is not dividable by 4.\nFor each sequence in your fastq files, you must have"
 ...
 my $path_out_read1 = $dir_read1 . "/" . $read1_name_without_suffix . ".sort.fq";
 open(FH_OUT1, ">$path_out_read1");
 my $dir_read2 = dirname($path_read2);
 my $read2_name_without_suffix = suffix_remover($path_read2);
 my $path_out_read2 = $dir_read2 . "/" . $read2_name_without_suffix . ".sort.fq";
 open(FH_OUT2, ">$path_out_read2");    
 my $path_out_read1_singletons = $dir_read1 . "/" . $read1_name_without_suffix . ".singletons.fq";
...
if (exists $read2{$key}){    
 ...
 my $path_out_read2_singletons = $dir_read2 . "/" . $read2_name_without_suffix . ".singletons.fq";
