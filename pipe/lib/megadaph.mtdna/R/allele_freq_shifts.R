...
prop_test_across_genome <- function(in_list) {
    ...
    # Output data frame, containing locus position (pos), minimum and maximum
    # frequency between genotypes (minfreq, maxfreq), p-value for difference
    # (p), and confidence intervals (conf1,2)
    pvals = data.frame(pos=numeric(0),
                       minfreq=numeric(0),
                       maxfreq=numeric(0),
                       p=numeric(0),
                       conf1=numeric(0),
